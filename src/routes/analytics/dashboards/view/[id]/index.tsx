...
export const useDashboardData = routeLoader$(async (requestEvent) => {
  ...
  try {
    ...
    try {
      const execResponse = await ssrApiClient.post(`/dashboards/${dashboardId}/execute`, {});
      ssrWidgetResults = normalizeDashboardWidgetResults(execResponse, dashboard.widgets || []);
    } catch {
      // Fallback: client-side execution will run on mount.
    }

    return { dashboard, widgetResults: ssrWidgetResults };
  } catch (error: any) {
    return {
      dashboard: null,
