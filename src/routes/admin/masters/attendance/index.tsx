  ...
  return (
    <div class="space-y-6 py-2">
        <PageHeader
          ...
          subtitle={`Live tracking and attendance audit for ${businessCode.value || 'current business'}`}
        >
          <Btn q:slot="actions" variant="secondary" onClick$={() => nav('/admin/masters/sites')}>
            Sites
          </Btn>
        ...
        {selectedUserId.value && (
          <SectionCard>
            ...
            {timelineLoading.value ? (
              <div class="text-neutral-500 text-sm">Loading timeline...</div>
            ) : timelineError.value ? (
              <div class="text-error-700 text-sm">{timelineError.value}</div>
            ) : timelineSessions.value.length === 0 ? (
              <div class="text-neutral-500 text-sm">No timeline entries found.</div>
            ) : (
              <div class="space-y-3">
                  ...
                  return (
                    <div key={session.id} class="rounded-lg border border-neutral-200 p-4">
                      <div class="flex items-center justify-between gap-3 flex-wrap">
                        <div>
                          <div class="font-semibold text-neutral-800">{session.site?.name || session.siteId}</div>
