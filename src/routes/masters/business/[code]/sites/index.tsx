...
export default component$(() => {
  ...
  return (
    <div class="space-y-6 py-2">
        <PageHeader title="Sites" subtitle={`Manage sites for ${businessCode}`}>
          <Btn q:slot="actions" variant="secondary" onClick$={() => nav(`/masters/business/${businessCode}/attendance`)}>
            Attendance Monitor
          </Btn>
          <Btn q:slot="actions" onClick$={() => nav(`/masters/business/${businessCode}/sites/new`)}>
            + Add Site
          </Btn>
        </PageHeader>
        ...
        <SectionCard>
          {sites.value.length === 0 ? (
            <div class="text-center py-12">
              ...
              <Btn
                onClick$={() => nav(`/masters/business/${businessCode}/sites/new`)}
              >
                Add Site
              </Btn>
            </div>
              ...
              <DataTable>
                ...
                <DataTableBody>
                  {sites.value.map((site) => (
                    <DataTableRow key={site.id} class="hover:bg-neutral-50 transition">
                      <DataTableCell>
                      ...
                      <DataTableCell>
                        <span class="text-xs font-mono text-neutral-700">{site.code}</span>
                      </DataTableCell>
                      <DataTableCell>
                        {site.location ? (
                      ...
                      <DataTableCell class="text-right">
                        <div class="flex justify-end gap-2">
                          <Btn
                            onClick$={() => nav(`/masters/business/${businessCode}/sites/${site.id}/access`)}
                            variant="ghost"
                            size="sm"
                            title="Manage Access"
                          >
                            <i class="i-heroicons-lock-closed-solid h-4 w-4 inline-block" aria-hidden="true"></i>
                          </Btn>
                          <Btn
                            onClick$={() => nav(`/masters/business/${businessCode}/sites/${site.id}/edit`)}
                            variant="ghost"
                            size="sm"
                            title="Edit"
                          >
                            <i class="i-heroicons-pencil-square-solid h-4 w-4 inline-block" aria-hidden="true"></i>
                          </Btn>
                          <Btn
                            ...
                            title="Delete"
                          >
                            <i class="i-heroicons-trash-solid h-4 w-4 inline-block text-white" aria-hidden="true"></i>
                          </Btn>
                        </div>
                      </DataTableCell>
                    </DataTableRow>
